...
// import { PowerIcon, HomeIcon, UserPlusIcon, LightBulbIcon } from '@heroicons/react/24/outline';

// const Home = () => {
//     const [devices, setDevices] = useState([]);
//     const [homes, setHomes] = useState([]); // Vẫn giữ mảng homes để phục vụ select box
    
//     // State cho Form tạo nhà mới
...
//                 const userHomeId = result.data.homeId;
//                 // Tạo một mảng giả lập để Select Box vẫn hoạt động bình thường
//                 setHomes([{ _id: userHomeId, name: "Nhà của tôi" }]);
...
//     // 3. Tạo nhà mới (Dành cho Admin chưa có nhà)
//     const handleCreateHome = async (e) => {
//         e.preventDefault();
//         try {
//             const res = await fetch("http://localhost:3000/api/v1/homes", {
//                 method: "POST",
//                 headers: {
...
//                 setNewHomeName("");
//                 fetchUserHomeInfo(); // Refresh lại thông tin nhà để lấy ID mới
//                 alert("Đã tạo không gian nhà thành công!");
//             } else {
//                 alert("Lỗi: " + (result.error || "Không thể tạo nhà"));
//             }
//         } catch (error) {
...
//         e.preventDefault();
//         if (!selectedHomeId) {
//             setInviteMessage({ type: 'error', text: 'Vui lòng chọn nhà trước khi mời' });
//             return;
//         }
...
//         try {
//             // URL Mới dựa theo OpenAPI: /api/v1/homes/{homeId}/members/invite
//             const res = await fetch(`http://localhost:3000/api/v1/homes/${selectedHomeId}/members/invite`, {
//                 method: "POST",
//                 headers: {
//                     "Content-Type": "application/json",
//                     "Authorization": `Bearer ${token}`
...
//                 setInviteMessage({ type: 'success', text: `Đã mời ${inviteEmail} thành công!` });
//                 setInviteEmail("");
//             } else {
...
//     // 5. Điều khiển Bật/Tắt thiết bị
//     const toggleDevice = async (deviceId, currentStatus) => {
//         // Theo OpenAPI: enum là "Bật" hoặc "Tắt"
//         const newValue = currentStatus === "Bật" ? 0 : 1; 
        
//         // Tối ưu UI: Đổi trạng thái hiển thị ngay lập tức (Optimistic Update)
//         setDevices(devices.map(d => 
//             d._id === deviceId ? { ...d, status: newValue === 1 ? "Bật" : "Tắt" } : d
//         ));

//         try {
//             // URL Mới: /api/v1/devices/{id}/control
...
//             if (!res.ok) {
//                 // Nếu gọi API thất bại, trả lại trạng thái cũ bằng cách fetch lại data gốc
...
//     if (isLoading) return <div className="p-9 text-center text-gray-500">Đang tải dữ liệu...</div>;
...
//                 {/* CỘT TRÁI: DANH SÁCH THIẾT BỊ (Chiếm 2/3) */}
...
//                                 {devices.map((device) => (
//                                     <div key={device._id} className="border border-gray-200 rounded-lg p-4 flex justify-between items-center bg-gray-50 hover:bg-white hover:shadow-md transition duration-200">
//                                         <div className="flex items-center gap-3">
//                                             {/* Cập nhật check có dấu "Bật" */}
...
//                 {/* CỘT PHẢI: QUẢN LÝ NHÀ & THÀNH VIÊN (Chiếm 1/3) */}
//                 <div className="space-y-6">
                    
//                     {/* Block 1: Mời thành viên */}
//                     <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
//                         <div className="flex items-center gap-2 mb-4">
//                             <UserPlusIcon className="w-6 h-6 text-blue-600" />
//                             <h2 className="text-lg font-semibold text-gray-800">Chia sẻ quyền</h2>
//                         </div>
                        
//                         {homes.length === 0 ? (
//                             <p className="text-sm text-gray-500">Bạn cần tạo một Ngôi nhà trước khi có thể mời người khác.</p>
//                         ) : (
...
//                                         className="w-full border border-gray-300 rounded p-2 focus:ring-blue-500 text-sm bg-gray-50"
//                                     >
//                                         {homes.map(home => (
...
//                                 </div>
//                                 <div>
//                                     <label className="block text-sm font-medium text-gray-700 mb-1">Email người thân</label>
//                                     <input 
//                                         type="email" required
...
//                                 {inviteMessage.text && (
//                                     <p className={`text-xs font-medium ${inviteMessage.type === 'error' ? 'text-red-600' : 'text-green-600'}`}>
//                                         {inviteMessage.text}
//                                     </p>
//                                 )}

//                                 <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 rounded transition text-sm">
//                                     Gửi lời mời
//                                 </button>
//                             </form>
//                         )}
...
//                             <h2 className="text-lg font-semibold text-gray-800">Tạo không gian mới</h2>
//                         </div>
//                         {homes.length > 0 ? (
//                             <p className="text-sm text-gray-500">Tài khoản này đã sở hữu một Ngôi nhà. Bạn không thể tạo thêm.</p>
//                         ) : (
//                             <form onSubmit={handleCreateHome} className="space-y-4">
//                                 <div>
//                                     <input 
//                                         type="text" required
//                                         value={newHomeName} onChange={(e) => setNewHomeName(e.target.value)}
//                                         placeholder="Tên nhà (VD: Nhà Đà Nẵng)"
//                                         className="w-full border border-gray-300 rounded p-2 focus:ring-green-500 text-sm"
//                                     />
//                                 </div>
//                                 <button type="submit" className="w-full bg-green-50 text-green-700 border border-green-200 hover:bg-green-100 font-medium py-2 rounded transition text-sm">
//                                     Tạo Nhà
//                                 </button>
//                             </form>
//                         )}
